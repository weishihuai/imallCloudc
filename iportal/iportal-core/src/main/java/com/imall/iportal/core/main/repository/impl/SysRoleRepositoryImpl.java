...
import com.imall.iportal.core.main.repository.SysRoleRepositoryCustom;
...
public class SysRoleRepositoryImpl implements SysRoleRepositoryCustom {
    ...
    public Page<SysRole> query(Pageable pageable,Long orgId) {
        ...
        if (orgId!=null) {
            ...
            paramMap.put(SysRole.ORG_ID,orgId);
        }
        sql.append("  and IS_DEFAULT_ADMIN='N'  ");
        sql.append(" order by ORDERBY desc ");
        Query query = entityManager.createNativeQuery("SELECT * FROM T_PT_SYS_ROLE a where 1=1" + sql, SysRole.class);
        CommonUtil.formatQueryParameter(query, paramMap);
        query.setFirstResult(pageable.getPageNumber() * pageable.getPageSize());
        query.setMaxResults(pageable.getPageSize());
        List<SysRole> resultList = query.getResultList();

        Query totalQuery = entityManager.createNativeQuery("SELECT COUNT(*) FROM T_PT_SYS_ROLE a WHERE 1=1" + sql);
