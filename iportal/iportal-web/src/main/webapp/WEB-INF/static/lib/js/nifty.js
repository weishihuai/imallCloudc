		...
		init = function(){
			if (opt.container === 'page') {
				...
				if (!floatContainer[opt.floating.position]) {
					//TODO JINCHI 添加了z-index:999999999 ，保证弹层显示在所有窗口最顶端
					floatContainer[opt.floating.position] = $('<div style="z-index:999999999;" id="floating-' + opt.floating.position + '" class="floating-container"></div>');
...
!function ($) {
	...
	updateSize = function(){
		...
		if (screenCat != currentScreen){
			...
			if(nifty.screenSize == 'sm' && nifty.container.hasClass('mainnav-lg')){
				$.niftyNav('expand');//TODO 原值collapse
			}
		}
	},
	...
	//TODO nifty load加载在chrome下加载菜单第二次事件失效，改用jquery的加载方式
	$(function(){
