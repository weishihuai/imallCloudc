...
class DrugCheckAddForm extends  Component{
    constructor(props) {
        super(props);
    }
    ...
    componentDidMount() {
        ...
        $(".datepicker").on("click",function(e){
            ...
            $(this).lqdatetimepicker({
                ...
                dateType : 'D',
                selectback : function(){
                    let planCheckTimeString = $("#planCheckTimeString").val().trim();
                    change('planCheckTimeString', planCheckTimeString);
                }
            });
        });
    }
    ...
    valid(data){
        ...
        let drugCheckItemSaveVoList = [];
        goods.map((item)=>{
            drugCheckItemSaveVoList.push({'goodsId':item.goodsId, 'goodsBatchId':item.id});
        });

        const {params} = store.getState().todos;
        const {actions} = this.props;
        data = Object.assign({},data,{
            drugCheckItemSaveVoList: drugCheckItemSaveVoList
        });
     ...
     */
    openGoodsSelectWin(){
        this.props.changeCommonGoodsBatchListState({isSingle: true, gtInStockDays: 30, isBiggerThanCurrentStock: 'Y', isInEffective: 'Y', batchState: 'NORMAL'}, (goods) => this.selectGoods(goods));
    }
                        ...
                        <div className="layer-body">
                            <div className="md-box">
                                ...
                                <div className="box-mc clearfix">
                                    <Field id="planCheckTimeString" name="planCheckTimeString"  type="text" component={inputField} label="计划检查时间" inputClassName="datepicker" readOnly="readOnly" required="required"/>
                                    <Field id="checkTypeCode" name="checkTypeCode"  type="text" component={selectField} label="检查类型" className="select-w" optionValue="code" optionName="name" items={checkTypeCode} required="required"/>
                                </div>
                            </div>
                            <div className="md-box">
                                ...
                                <div className="box-mc">
                                    <table className="w1075">
                                        ...
                                        <tr>
                                            ...
                                            <th className="time" style={{'width': 80 + 'px'}}>生产日期</th>
                                            <th className="time" style={{'width': 80 + 'px'}}>有效期至</th>
...
DrugCheckAddForm.propTypes = {
    handleSubmit: PropTypes.func.isRequired,
    submitting: PropTypes.bool.isRequired
};

DrugCheckAddForm.contextTypes = {
...
DrugCheckAddForm = reduxForm({
    form: 'drugCheckAddForm'
})(DrugCheckAddForm);

DrugCheckAddForm = connect(
    mapStateToProps,
    mapDispatchToProps
)(DrugCheckAddForm);

export default DrugCheckAddForm;
