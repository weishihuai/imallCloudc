...
class BadReactionRepUpdateForm extends React.Component {

    componentWillMount() {
        this.props.actions.badReactionRepDrugInfData(this.context.store.getState().todos.data.id);
    }
    ...
    render() {
        ...
        let badReactionRepDrugInfData = store.getState().todos.badReactionRepDrugInfData || {};
        let suspectDrugInfList = badReactionRepDrugInfData.suspectDrugInfList || [];  //怀疑药品信息
        let blendDrugInfList = badReactionRepDrugInfData.blendDrugInfList || [];  //并用药品信息
                ...
                <div className="layer">
                    <div className="layer-box layer-info layer-order layer-adverse-reactions w1617">
                        <div className="layer-header">
                            <span>修改不良反应报告</span>
                            <a href="javascript:void(0);" className="close" onClick={() => actions.badReactionRepUpdateModel(false)}/>
                        </div>
                            ...
                            <div className="md-box">
                                <div className="box-mc clearfix">
                                    <table>
                                        ...
                                        {
                                            suspectDrugInfList.map((field, index) => {
                                                return (
                                                    <tr key={index}>
                                                        {index === 0 && <td rowSpan={suspectDrugInfList.length}><span style={{textAlign: "center"}}>怀疑药品</span></td>}
                                                        <td style={{display:"none"}}><Field id={"suspectDrugList[" + index + '].id'} name={"suspectDrugList[" + index + '].id'} type="hidden" component={hiddenField} /></td>
                                        ...
                                        {
                                            blendDrugInfList.map((field, index) => {
                                                return (
                                                    <tr key={index}>
                                                        {index === 0 && <td rowSpan={blendDrugInfList.length}><span style={{textAlign: "center"}}>并用药品</span></td>}
                                                        <td style={{display:"none"}}><Field id={"blendDrugList[" + index + '].id'} name={"blendDrugList[" + index + '].id'} type="hidden" component={hiddenField} /></td>
                        ...
                        <div className="layer-footer">
                            ...
                            <a href="javascript:void(0);" className="cancel" onClick={() => actions.badReactionRepUpdateModel(false)}>取消</a>
                        </div>
                    </div>
                </div>
            </form>
...
BadReactionRepUpdateForm.contextTypes = {
    store: React.PropTypes.object
};

BadReactionRepUpdateForm.propTypes = {
    handleSubmit: PropTypes.func.isRequired,
    submitting: PropTypes.bool.isRequired
};

BadReactionRepUpdateForm = reduxForm({
    form: 'badReactionRepUpdateForm',
    enableReinitialize: true,
})(BadReactionRepUpdateForm);
...
function mapStateToProps(state) {
    return {
        initialValues: Object.assign({},  state.todos.data ,{
            suspectDrugList:state.todos.badReactionRepDrugInfData.suspectDrugInfList || [],
            blendDrugList:state.todos.badReactionRepDrugInfData.blendDrugInfList || [],
        }),
        state,
        validate
    };
...
export default connect(mapStateToProps, mapDispatchToProps)(BadReactionRepUpdateForm);