...
class GoodsDocUpdateForm extends Component {
    ...
    componentDidMount() {
        ...
        $target.attr('data-id', detailData.goodsCategoryId);
    }
    ...
    submit(data){
        ...
        const {params,isUpdateToWaitApprove} = store.getState().todos;
        let pictFileList = store.getState().todos.pictFileList||[];
        let otherFileList = store.getState().todos.otherFileList||[];
        let newPictFileList = [];
        ...
        return this.props.actions.saveOrUpdateGoods(Object.assign({},data,{
            ...
            isUpdateToWaitApprove:isUpdateToWaitApprove,
    ...
    saveAndUpdateToWaitApprove(){
        this.props.checkValidForm(true);
        this.context.store.dispatch({
            type:types.GOODS_DOC_IS_UPDATE_TO_WAIT_APPROVE,
            data:"Y"
        })
    }
    ...
    selectManagementCategory() {
        ...
        this.props.showArticleCategoryModal(true, (ids, objs) => {
            if (objs.length == 0) {
                ...
                let formData = store.getState().form.goodsDocUpdateForm.values;
                let articleData = Object.assign({}, formData, {
                    goodsCategoryId: goodsCategoryId,
                    pathName: pathName
                });
                store.dispatch({
                    type: types.GOODS_DOC_UPDATE_SET_GOODS_CATEGORY,
                    data: articleData
                });
            }
                ...
                <div className="layer">
                    ...
                    <div className="layer-box layer-info w960">
                        <div className="layer-header">
                            <span>修改商品档案</span>
                            <a href="javascript:void(0);" className="close" onClick={()=>actions.changeGoodsDocUpdateFormState(false)}/>
                        </div>
                        ...
                        <div className="layer-footer">
                            {detailData.approveStateCode=="DENY_APPROVE"&&<button href="javascript:void(0);" className="confirm" style={{border:"none",width:"136px"}} disabled={submitting} type="submit" onClick={() =>this.saveAndUpdateToWaitApprove()}>保存并提交审核</button>}
                            ...
                            <a href="javascript:void(0);" className="cancel" onClick={()=>actions.changeGoodsDocUpdateFormState(false)}>取消</a>
                        </div>
...
GoodsDocUpdateForm.propTypes = {
    handleSubmit: PropTypes.func.isRequired,
    submitting: PropTypes.bool.isRequired
};
GoodsDocUpdateForm.contextTypes = {
...
GoodsDocUpdateForm = reduxForm({
    form: 'goodsDocUpdateForm',
...
GoodsDocUpdateForm = connect(
    mapStateToProps,
    mapDispatchToProps
)(GoodsDocUpdateForm);

export default GoodsDocUpdateForm