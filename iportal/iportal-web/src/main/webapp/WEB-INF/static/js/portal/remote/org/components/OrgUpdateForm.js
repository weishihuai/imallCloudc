...
import {portalOrgUpdateModal} from '../actions';
import {PORTAL_CHANGE_PROVINCE,PORTAL_CHANGE_CITY} from '../constants/ActionTypes';
import {validate} from '../../../../common/common-validate';

export const fields = ['id','parentId','nodeCode', 'orgName', 'orgCode', 'zoneCode' ,'address','phone','fax','orderby','isAvailable']

class OrgUpdateForm extends Component {
    ...
    render() {
        const { portalOrgUpdateModal,fields: {id,parentId,nodeCode,orgName, orgCode, zoneCode ,address,phone,fax,orderby,isAvailable}, resetForm, handleSubmit, submitting } = this.props
        const {store} = this.context;
        return (
            <form className="form-horizontal" onSubmit={handleSubmit}>
                <input  className="form-control" type="hidden" {...parentId}/>
                <input  className="form-control" type="hidden" {...id}/>
                <input  className="form-control" type="hidden" {...nodeCode}/>
                ...
                <div className="form-group">
                    ...
                    <div className="col-md-9">
                        <div className="col-md-3">
                            <select name="zoneCode" id="zoneCode" className="form-control" defaultValue={store.getState().todos.provinceZoneCode} onChange={(e) => this.provinceHandle(e,store)}>
                        ...
                        <div className="col-md-4">
                            <select name="city" id="city" className="form-control" defaultValue={store.getState().todos.cityZoneCode} onChange={(e) => this.cityHandle(e,store)}>
                                <option  value="0">--请选择--</option>
                ...
                <div className="modal-footer" style={{"background":"none"}}>
                    ...
                    <button className="btn btn-default ngdialog-custom-close" type="button" disabled={submitting} onClick={portalOrgUpdateModal}>
                        关闭
                    </button>
                </div>
            </form>
...
OrgUpdateForm.propTypes = {
    fields: PropTypes.object.isRequired,
    handleSubmit: PropTypes.func.isRequired,
    resetForm: PropTypes.func.isRequired,
    submitting: PropTypes.bool.isRequired
}
OrgUpdateForm.contextTypes = {
...
OrgUpdateForm = reduxForm({
    form: 'synchronousValidation',
    fields,
    validate
})(OrgUpdateForm);

OrgUpdateForm = connect(
    mapStateToProps,
    mapDispatchToProps
)(OrgUpdateForm);
