...
export function saveOrUpdate(data, actions, params){
    ...
    if(data.id===undefined) {//新增
        isAdd = true;
        url = iportal + "/platform/drugCombinationCategory/save.json";
    }else{//更新
        isAdd = false;
        url = iportal + "/platform/drugCombinationCategory/update.json";
    }
...
export function drugCombinationCategoryList(params, page, size) {
    return function (dispatch) {
        return fetch(iportal + '/platform/drugCombinationCategory/query.json?categoryNm='+ params.categoryNm + '&page=' + page + '&size=' + size , {
            mode: 'cors',
            credentials: 'include',
            method: 'get'
        }).then(function (response) {
...
export function del(id, params){
    return function (dispatch) {
        ...
        return fetch(iportal + '/platform/drugCombinationCategory/delete.json', {
            mode: 'cors',
            credentials: 'include',
            method: 'POST',
            redirect: 'follow',
...
export function checkByCategoryNm(values, dispatch, props, blurredField) {
    return new Promise((resolve, reject) => {
        ...
        if(values.categoryNm!==undefined && values.categoryNm!==null && values.categoryNm!==''){
            const id = values.id ? values.id : '';
            fetch(iportal + '/platform/drugCombinationCategory/checkByCategoryNm.json?id=' + id + '&categoryNm=' + values.categoryNm, {
                mode: 'cors',
                credentials: 'include',
                method: 'get'
            }).then(function (response) {
