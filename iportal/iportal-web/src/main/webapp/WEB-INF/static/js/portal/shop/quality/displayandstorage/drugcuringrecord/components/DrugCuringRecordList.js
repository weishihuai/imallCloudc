...
import DrugCuringRecordSearchForm from "./DrugCuringRecordSearchForm";

class DrugCuringRecordList extends Component{
    constructor(props) {
        super(props)
    }
    ...
    componentWillMount() {
        ...
        this.props.actions.drugCuringRecordList(params, params.page, params.size);
    }
    ...
    onSizePerPageList(sizePerPage) {
        const {store} = this.context;
        const {params} = store.getState().todos;
        this.props.actions.drugCuringRecordList(params, params.page, sizePerPage);
    }

    onPageChange(page, sizePerPage) {
        const {store} = this.context;
        const {params} = store.getState().todos;
        this.props.actions.drugCuringRecordList(params, page - 1, sizePerPage);
    }
                ...
                <div className="main">
                    <div className="mt">
                        <div className="mt-lt">
                            ...
                            <span>></span>
                            <a href="javascript:void(0);">药品养护记录</a>
                            <DrugCuringRecordSearchForm actions={actions}/>
                        </div>
                        <div className="mt-rt">
                        </div>
                    </div>
                    <div className="mc">
                        <div className="table-box">
                            <table>
                                <thead>
                                <tr>
                                    <th className="checklists-code" style={{width: 125 + 'px'}}>养护计划单号</th>
                                    <th className="time">养护日期</th>
                                    <th className="name" style={{width: 100 + 'px'}}>养护人</th>
                                    ...
                                    <th className="batch-number" style={{width: '100px'}}>批号</th>
                                    <th className="time" >生产日期</th>
                                    <th className="time" >有效期至</th>
                                    <th className="goods">货位</th>
                                    <th className="goods">库存数量</th>
                                    <th className="goods">养护数量</th>
                                    <th className="check-project">养护项目</th>
                                ...
                                }
                                {
                                    record.map((record, index) => {
                                        return (
                                            <tr key={index}>
                                                <td>
                                                    <div className="td-cont" title={record.curingPlanNum}>{record.curingPlanNum}</div>
                                                </td>
                                                <td>
                                                    <div className="td-cont" title={record.curingDateString}>{record.curingDateString}</div>
                                                </td>
                                                <td>
                                                    <div className="td-cont" title={record.curingMan}>{record.curingMan}</div>
                                                </td>
                                                <td>
                                                ...
                                                <td>
                                                    <div className="td-cont" title={record.curingQuantity}>{record.curingQuantity}</div>
                                                </td>
                                                <td>
                                                    <div className="td-cont" title={record.curingPrj}>{record.curingPrj}</div>
                                                </td>
                                                <td>
...
DrugCuringRecordList.propTypes = {
    actions: PropTypes.object.isRequired
};

DrugCuringRecordList.contextTypes = {
...
export default connect(mapStateToProps, mapDispatchToProps)(DrugCuringRecordList);