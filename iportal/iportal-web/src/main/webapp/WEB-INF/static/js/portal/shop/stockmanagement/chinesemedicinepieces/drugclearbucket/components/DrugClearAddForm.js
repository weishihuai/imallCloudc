...
export const validate = (values, props) => {
    ...
    const errors = {};
    if(!values.clearBucketTimeString){
        errors.clearBucketTimeString = "清斗时间必填";
        props.errorValidMessageFunction(errors.clearBucketTimeString);
        return errors;
    }
...
export const goodsFieldArray = ({fields, allGoods}) => (
    <tbody>
    {
        allGoods.map((goods, index) => {
            return (
                <tr key={index}>
                    ...
                    <Field name={'goodsList[' + index + '].storageSpaceId'} component={hiddenInputField}/>
                    ...
                    <td>
                        <div className="td-cont">{goods.storageSpaceNm || "暂无"}</div>
                    </td>
...
class DrugClearAddForm extends React.Component {
    ...
    componentDidMount() {
        ...
        $(".datepicker").on("click", function (e) {
            ...
            $(this).lqdatetimepicker({
                ...
                dateType: 'D',
                selectback: function () {
                    change("clearBucketTimeString", $('input[name="drugTime"]').val());
                }
            });
        });
    }
    ...
    openCommonGoodsSelectWin() {
        ...
        this.props.changeCommonGoodsBatchListState({isSingle:false,isChineseMedicinePieces:"Y",isBiggerThanCurrentStock:"Y",isVirtualWarehouse:"N"}, (selectedGoodsList) => {
            const {store} = this.context;
            const goodsList = store.getState().form.drugClearAddForm.values.goodsList;
            let goodsIdArr = [];
            goodsList.map(goods => {
    ...
    fingApproveValidateData(data){
        this.props.change("approveDataId", data.id);
        this.props.actions.fingApproveValidateData(data);
    }

    resetAndClose(){
        this.props.actions.drugClearBucketAddModel(false);
        this.props.reset();
        this.props.actions.fingApproveValidateData({});
    }
    saveAndReset(){
                            ...
                            <div className="md-box">
                                ...
                                <div className="box-mc clearfix">
                                    ...
                                    <div className="item" style={{clear: "both"}}>
                                        <p><i>*</i>审核人</p>
                                        {approveData.id != undefined ? <p style={{color: "red"}}>{approveData.realName}</p> : <a  href="javascript:void(0);" onClick={() => { showValidateModel()}} className="review">点击审核</a>}
                                    </div>
                                </div>
                            </div>
                            <div className="md-box">
...
DrugClearAddForm.contextTypes = {
    store: React.PropTypes.object
};

DrugClearAddForm.propTypes = {
    handleSubmit: PropTypes.func.isRequired,
    submitting: PropTypes.bool.isRequired
};

DrugClearAddForm = reduxForm({
    form: 'drugClearAddForm'
})(DrugClearAddForm);
...
export default connect(mapStateToProps, mapDispatchToProps)(DrugClearAddForm);