...
<head>
    ...
    <script>
        ...
        function getSucceedUsers() {
            var passwordInput = $("#passwordInput").val();
            var password = $.md5(passwordInput);
            $.ajax({
                type:"GET",
                url:"${iportal}/shiftRecord/findNormalByShopId.json",
                data:{username:$("#username").val(), password:password, validateCode:$("#validateCode").val()},
                dataType: "json",
                success:function(data) {
                    var select = document.getElementById("succeedWhoId");
                    select.innerHTML = '<option value="">接谁的班？</option>';
                    data.userList.map(function (user) {
                        var option = document.createElement("option");
                        option.value = user.id;
                        option.innerHTML =  user.realName;
                        if(data.succeedWhoId && data.succeedWhoId===user.id){
...
<div class="main">
    <div class="mt">
        <h5>乐商销售POS前端系统</h5>
        <span>iMall salse pos system</span>
    </div>
    <div class="mc">
        <form id="loginForm" action="" method="post" onsubmit="return validation()">
            <div class="cont" style="height: 282px;">
                <div class="item">
                    <div class="pic"><img src="${iportal}/static/img/user30x30.png" alt=""></div>
                    <input type="text" class="form-control" id="username" name="username" placeholder="用户名" value="<shiro:principal/>"  onchange="getSucceedUsers()"/>
                </div>
                <div class="item">
                    ...
                    <input type="hidden" id="password" name="password" onchange="getSucceedUsers()">
                </div>

                <div class="item">
                    <div class="pic"><img src="${iportal}/static/img/code30x30.png" alt=""></div>
                    <input id="validateCode" name="validateCode" maxlength="4" type="text" class="input_bg" style="width:155px;" tabindex="4" placeholder="验证码" onchange="getSucceedUsers()"/>
                ...
                <div class="item">
                    <div class="pic"><img src="${iportal}/static/img/user30x30.png" alt=""></div>
                    <select class="form-control" id="succeedWhoId" name="succeedWhoId">
                        <option value="">接谁的班？</option>
                    </select>
                </div>
